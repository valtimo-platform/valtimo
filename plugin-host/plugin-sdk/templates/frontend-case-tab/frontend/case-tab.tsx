...
function CaseTab() {
    ...
    sdk
      ...
      .then((res) => {
        if (res.status >= 200 && res.status < 300) {
          ...
          setSummary({state: "error", message: sdk.t("caseTab.error")});
        }
      })
  ...
  return (
    <div style={{fontFamily: "IBM Plex Sans, sans-serif"}}>
      <h2>{sdk.t("caseTab.title")}</h2>
      {summary.state === "loading" && <p>{sdk.t("caseTab.loading")}</p>}
      {summary.state === "error" && <p>{summary.message}</p>}
      {summary.state === "ready" && (
        <>
          <p>{summary.data.message}</p>
          <p>{summary.data.documentId ?? "—"}</p>
        </>
      )}
    </div>
  );
...
sdk.ready().then(() => {
  sdk.emit("ready", {});
  createRoot(document.getElementById("root")!).render(<CaseTab />);
});
