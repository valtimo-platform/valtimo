# External Plugin System — Plan v17
...
The platform aims to ship the external-plugin module with **no additional `application.yml`
entries** beyond what core Valtimo already requires. Every value the module needs is either:

- **Per-host**, entered once in the add-host UI (host base URL, admin token, callback URL, optional
  broker URL/exchange) and stored on the host row, **or**
...
|----------------------|---------------------|------|
| JWT signing key | `SHA-256(valtimo.plugin.encryption-secret)` | At every JWT issue/verify; the hash makes the key always 32 bytes regardless of the secret's raw length, so AES-128 (16-byte) and AES-256 (32-byte) deployments both work |
| `gzacBaseUrl` per push | `external_plugin_host.gzac_callback_base_url` | Set in the add-host UI |
| Broker AMQP URL per push | `external_plugin_host.event_broker_amqp_url` (nullable) | Set in the add-host UI; null disables events for hosts under this host |
| Broker exchange per push | `external_plugin_host.event_broker_exchange`, else `valtimo.outbox.publisher.rabbitmq.exchange` | Set in the add-host UI; almost never overridden |
...
`SHA-256(valtimo.plugin.encryption-secret)` — see
`security/ExternalPluginServiceTokenKeyProvider.kt`. Hashing gives us domain separation as a side
effect: if a service token leaks, the AES encryption key cannot be recovered from it.
...
**3.7 Caveat** — service tokens bypass PBAC, so the allowlist is the entire authorization surface; an
over-broad grant (`/api/v1/**`) gives broad role-free access. Hence the activation-time acceptance
screen (§4) is security-critical.
...
- `external_plugin_host` — `base_url`, encrypted `secret`, `status`, health/failure counters,
  **plus** `gzac_callback_base_url`, `event_broker_amqp_url`, `event_broker_exchange` (all populated
  from the add-host UI; the two broker columns nullable for events-off / use-default-exchange).
...
Events add **no new table**: subscriptions come from `manifest_json`, the broker connection details
come from the host row, and at push time they are pushed transiently to the host (held only in the
host's in-memory registry until the host stores them in its own PostgreSQL).

## 6. Adding a host & host-defaults endpoint ✅

`GET /api/management/v1/external-plugin/host-defaults` (`ExternalPluginManagementResource`) returns
three pre-fills the add-host UI uses to populate the new-host form:
...
Derivation:
- `gzacCallbackBaseUrl` = `http://localhost:{server.port}` (default port `8080`). We deliberately do
  **not** use the incoming request URL: the admin reaches GZAC through the Angular dev proxy on
  port 4200, or through a reverse proxy in production. Neither is the URL the plugin host should
  call back on — the host typically lives on the same Docker network as the backend and reaches it
  on the backend's native port. The default reflects "where the backend is listening", not "where
  the admin is browsing from".
- `eventBrokerAmqpUrl` = `amqp://{user}:{pass}@{host}:{port}{vhost}` built from `spring.rabbitmq.*`
  (GZAC's own broker view). Correct on machines where the host shares the broker's network.
- `eventBrokerExchange` = `valtimo.outbox.publisher.rabbitmq.exchange` (default `valtimo-events`).

The operator edits whatever does not match the host's network — for a host in another Docker
network, that's typically swapping `localhost` for the service name (`http://gzac:8080`,
`amqp://gzac-rabbitmq:5672`); for production it's the public URL behind the reverse proxy
(`https://gzac.example.com`). Three fields are exposed: `gzacCallbackBaseUrl` is required;
`eventBrokerAmqpUrl` and `eventBrokerExchange` are optional. Leaving the broker URL blank disables
events for every configuration under this host.

`POST /host` body shape:
...
`ExternalPluginHostService.register()` trims trailing `/` on the URLs, encrypts the secret, blanks
become `null`.
...
an in-memory registry **and persisted to PostgreSQL**. The only registered host function is `gzac_api`.
...
(`./plugins`), `LOG_LEVEL` (info), `HOST_ID` (defaults to the OS hostname; see §8.4), plus
`DB_HOST/PORT/NAME/USER/PASSWORD` for the host's PostgreSQL. **No broker variables** — the host
...
GZAC side. `ExternalPluginConfigurationService.pushToHost(config, definition, host)` reads the
broker fields **off the host row**:

```kotlin
hostClient.pushConfiguration(
    baseUrl = host.baseUrl,
    adminToken = encryptionService.decrypt(host.secret),
    configId = configuration.id.toString(),
    pluginId = definition.pluginId,
    pluginVersion = definition.version,
    properties = decryptedProperties(configuration),
    serviceToken = serviceTokenService.issue(configuration, definition),
...
- `host.gzacCallbackBaseUrl` is required at host creation; `fallbackGzacBaseUrl`
  (`http://localhost:{server.port}`) is only used for legacy rows.
- `host.eventBrokerAmqpUrl` is optional — when null, the `eventBroker` block is omitted from the
  push and events are simply not delivered for any configuration under this host (actions still
  work).
- `host.eventBrokerExchange` is optional — null falls back to `valtimo.outbox.publisher.rabbitmq.exchange`,
  which is what GZAC itself publishes to.
- `exchangeType` is hardcoded `fanout` — matches the declared exchange in
  `imports/gzac-rabbitmq/definitions.json`. No persistent override.

The body sent by `ExternalPluginHostClient.pushConfiguration()` to
`POST/PUT {host.baseUrl}/api/host/configurations/{configId}`:
...
```json
{
  ...
  "eventBroker": {
    "amqpUrl": "amqp://guest:guest@gzac-rabbitmq:5672",
...
(`brokerKey = amqpUrl + exchange + exchangeType`). After any configuration mutation the route calls
`sync()` (serialised via a promise chain): it opens consumers for newly referenced brokers and
closes consumers no configuration references any more. A `BrokerConsumer`:
...
On boot the host calls `eventConsumerManager.sync()` which re-opens consumers for every config that
still carries an `eventBroker.amqpUrl`. **Expect a `"Broker consumer started"` log line at startup
if any persisted configs reference a broker**, even before GZAC sends a fresh push.
...
instance A's events never reach instance B's configs) **and** (b) whose `manifest.eventSubscriptions`
contains the CloudEvent `type`. The Wasm `EventInput` is the flattened event
(`type, id, source, time, userId, roles, resultType, resultId, result`) plus the configuration's
`properties`. `serviceToken` and `gzacBaseUrl` ride in the Extism per-call `hostContext`, so an
event handler's `gzac_api` callback is authenticated and allowlist-enforced exactly like an action's.

Because the exchange is a fanout and each host binds its **own** queue:
...
- *One host serving multiple GZAC instances*: each instance has its own broker (different
  `amqpUrl`), so the host opens a separate `BrokerConsumer` per `brokerKey`. Dispatch only fires
  configurations whose pushed broker key matches the consuming connection — instance A's events
  never reach instance B's configs.
...
### 8.6 Worked example (`sample-plugins/case-summary`)

Subscribes to `document.created`, `task.completed`, `document.viewed`. On `document.created` it
writes a note back to the document:
...
`permissions.managementEndpoints` (`POST /api/v1/document/*/note`); the configuration must be
granted it at activation, and the allowlist filter (§3.4) permits the bearer-token call.

### 8.7 Event-type catalog

CloudEvent `type` strings are matched verbatim. Notable ones:
...
## 10. Deletion semantics & cascade ⛔ (to do — partial guards in place)
...
**Current state:**
- `DELETE /api/host/plugins/:id/:version` on the host (§7) refuses with **409 Conflict** if active
...
Discovery cleanup: when a definition disappears upstream (host no longer lists it),
`consecutiveMisses` increments per poll. After **N** misses (configurable, default 3) the
definition is marked `MISSING` rather than deleted; deletion is admin-driven once configurations
are also removed.

## 11. Multi-version support 🟡 (coexistence ✅, upgrade ⛔ to do)

**11.1 What works today.** The data model is multi-version capable:
...
5. Admin deletes the v1 configuration; v1 definition can then be removed (per §10).

**11.2 What's missing (to do):**
...
- **Process-link upgrade**: today every BPMN service task carries `external_plugin_version`. An
  in-place "upgrade configuration to v2" must rewrite affected process links, or — preferably —
  drop the `pluginVersion` column from `process_link` and resolve the version transitively through
  the configuration. That's a schema change worth taking when the upgrade flow lands.
- **Version status / channel**: add `ExternalPluginDefinitionStatus.{LATEST, STABLE, DEPRECATED}`
  so the activation screen can guide operators toward the recommended version and show a warning
  for deprecated ones. New configurations on a `DEPRECATED` version are blocked; existing ones keep
  working.
...
## 12. Logos and translations shipped with plugins ⛔ (to do)
...
**12.1 Package layout (new SDK convention):**
...
**12.2 Translation key conventions.** Locale files are flat JSON of `key → translation`. To avoid
...
`manifest.json` text fields (`name`, `description`, `actions[].title`,
`permissions.managementEndpoints[].description`) support a `{{ i18n:<key> }}` template syntax; the
management UI resolves these against the shipped bundle at render time, with the un-templated raw
string as fallback for unknown locales.

**12.3 Host side.** The host serves both via the existing bundle route, namespaced under `assets/`:
...
- `ExternalPluginTranslationService` (new) loads each active configuration's bundle into the
  `localization` module's runtime resource bundle at activation time, with the same `en` fallback
  as built-in modules. Cache TTL ≈ 5 minutes; invalidated on configuration mutation.
...
**12.5 UI surfaces** that pick up the new assets:
...
plugin asset bundle (logos + i18n, §12);
multi-version upgrade flow (§11.2);
full deletion-cascade contract with `force=true` admin override (§10).

## 14. Roadmap (priority order)

1. **Deletion cascade & guards (§10)** — block configuration delete when process links exist;
   block host delete when configurations exist; add `?force=true` admin override; structured 409
   bodies for the UI dependency list. ⛔
2. **Plugin assets — logo + i18n (§12)** — SDK package layout, host serving routes, core-app
   discovery + proxy + translation merge, UI surfaces. ⛔
3. **Multi-version upgrade UX (§11.2)** — version status (`LATEST/STABLE/DEPRECATED`), compatibility
...
4. Capabilities + host functions + allowlist enforcement; surface in the acceptance screen by
   category.
5. Durable/replayable event delivery option (per-host durable queue with TTL) for hosts that must
   not miss events during downtime.
6. Downscoped user tokens.
7. HTMX pages, case tabs/widgets, menu pages.
8. Host database (KV, API logs, retention) + admin log view.
9. Cleanup: align async-vs-sync SDK docs.
...
- `git diff next-minor -- backend/app/gzac/src/main/resources/application.yml`: empty
  (no application.yml additions over `next-minor`).
- Events, end-to-end against the live `gzac-rabbitmq` broker (sample plugin):
  - host startup re-opens consumers from persisted configs (the `"Broker consumer started"` log
    line is expected when a previously-pushed configuration is still in the host's PostgreSQL);
  ...
  - `document.created` → `gzac_api` issues `POST /api/v1/document/<id>/note` with the service-token
    bearer and body `{"content":"consumed by external plugin on <ISO date>"}`;
  - **Multiple hosts per instance**: two hosts with distinct `HOST_ID` each bind their own queue
    and both handle a single published event (two note POSTs); a replica with a shared `HOST_ID`
    joins the existing queue (no new queue) and the group handles each event once;
