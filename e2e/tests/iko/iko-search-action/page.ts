...
export class IkoSearchActionPage {
  ...
  /**
   * Fill the add-action modal (title → auto-key) and wait until the form is
   * valid (Save enabled). A freshly opened modal initialises its reactive form
   ...
   * Re-filling via `toPass` defeats both races.
   */
  async fillSearchActionForm(title: string): Promise<void> {
    await expect(async () => {
  ...
  async createSearchAction(title: string): Promise<string> {
    ...
    await this.fillSearchActionForm(title);
