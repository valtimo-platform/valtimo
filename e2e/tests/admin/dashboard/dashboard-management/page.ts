...
export class DashboardManagementPage {
  ...
  async selectCaseType(caseType: string) {
    const caseTypeDropdown = this.page
      .locator('valtimo-widget-configuration-container cds-dropdown')
      .first();
    await expect(caseTypeDropdown).toBeVisible({timeout: 10_000});

    const option = caseTypeDropdown.getByRole('listbox').getByText(caseType, {exact: true});

    // The case type options are populated by an async `getAllDefinitions()` call.
    // When the data source and case type are selected back-to-back (e.g. 10.27),
    // the dropdown can be opened before the options arrive. Carbon renders an
    // empty list that it does not repopulate while open, so close and reopen
    // until the option is present rather than waiting on a single open.
    await expect(async () => {
      await caseTypeDropdown.click(); // open
      if (await option.isVisible()) return;
      await this.page.keyboard.press('Escape'); // close so the next open re-renders
      throw new Error(`Case type "${caseType}" options not loaded yet`);
    }).toPass({timeout: 20_000, intervals: [500, 1_000, 1_500, 2_000]});
