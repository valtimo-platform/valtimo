...
When a process encounters an error during execution, Operaton logs the error and, depending on the configured [retry cycle](../retry-cycle.md), retries the failed task.
...
By default, Operaton does **not** log a dedicated message when a process enters an incident state due to exhausted retries.
...
This makes it possible to configure alerts or triggers based on log entries containing the text "OPERATON_INCIDENT_FAILED_JOB".

## Customizing the log message

You can customize the log message using the Spring configuration property `operaton.incident.alert-log.message-template`.
