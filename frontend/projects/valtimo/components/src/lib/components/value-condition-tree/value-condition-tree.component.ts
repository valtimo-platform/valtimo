...
/** The comparison operators a condition can use. `in` takes a comma-separated list; `exists` takes no value, or `false` to require absence. */
const DEFAULT_CONDITION_OPERATORS = ['==', '!=', '>', '>=', '<', '<=', 'in', 'contains', 'exists'];

/** Editor for a tree of `path / operator / value` rows, optionally wrapped in nestable AND/OR groups. Renders itself per nested group, so one component covers any depth. */
@Component({
  standalone: true,
...
export class ValueConditionTreeComponent {
  ...
  public addGroup(): void {
    // Seeded with one empty condition so the group starts usable; an empty group is dropped on serialization.
