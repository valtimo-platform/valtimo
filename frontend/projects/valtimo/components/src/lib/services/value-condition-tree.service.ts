...
/**
 * Translates a condition tree between its JSON form (a list of conditions and `allOf`/`anyOf` groups)
 * and the nested reactive form the tree editor works on, and back again.
 *
 * Every row in the form holds both a condition's and a group's controls; `kind` decides which of the
 * two is rendered and serialized. That keeps switching a row between the two lossless while editing.
 */
...
export class ValueConditionTreeService {
  ...
  private serializeNode(group: FormGroup): ValueConditionNode | null {
    if (group.get('kind')?.value === 'group') {
      ...
      // An empty group is dropped rather than sent: the backend rejects one, and a group the author
      // has not filled in yet should not block saving the rest of the plan.
