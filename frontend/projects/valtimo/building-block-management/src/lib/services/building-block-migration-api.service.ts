...
/**
 * The migration plans of one building block definition version.
 *
 * Everything but the URL is [BlueprintMigrationApiService]'s. There is no `start` and no dry run: a
 * building block plan is applied by the case migration that moves a building block onto its version,
 * and simulated by that case migration's dry run.
 */
