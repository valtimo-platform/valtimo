...
import {DossierProcessTimerService} from '../../../../services';
...
export class DossierDetailTabProgressComponent {
   ...
   * check is needed here. Checking the `complete` permission on `CamundaTimer` up front would not
  ...
  public readonly skippableTimers$: Observable<Array<SkippableTimer>> = combineLatest([
    ...
    switchMap(([documentId, processInstanceId]) => {
      ...
      return this.dossierProcessTimerService.getSkippableTimers(documentId, processInstanceId).pipe(
  ...
  constructor(
    ...
    private readonly dossierProcessTimerService: DossierProcessTimerService,
  ...
  public onConfirmSkipTimer(): void {
    ...
    */
    ...
    this.dossierProcessTimerService
