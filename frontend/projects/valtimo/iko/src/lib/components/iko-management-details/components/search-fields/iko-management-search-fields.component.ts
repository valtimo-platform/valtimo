...
export class IkoManagementSearchFieldsComponent implements OnInit, OnDestroy {
  ...
  private readonly _searchAction$: Observable<IkoSeachActionResponse> = this.params$.pipe(
    switchMap((params: {aggregateKey: string; actionKey: string}) =>
      this.ikoManagementApiService.getIkoSeachAction(params.aggregateKey, params.actionKey)
    )
  );
