...
class BuildingBlockProcessVersionCheckerTest {
    ...
    fun `should pass when the plan renamed the process and moved it onto the new key's definition`() {
        // The plan's processMigration mapped `verhuizing-inspectie-process` onto a differently keyed
        // process, so the running process now carries the *new* key and the link for it must match.
