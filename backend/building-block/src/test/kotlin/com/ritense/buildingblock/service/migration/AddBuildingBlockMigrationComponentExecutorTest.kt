...
/** The two ways `addBuildingBlock` can create nothing. Both used to be a bare return, so a plan in either state migrated every case and reported success. */
class AddBuildingBlockMigrationComponentExecutorTest {
    ...
    fun `should warn once for the component, not once per unreached entry`() {
        // The volume this replaced: 46 lines for one case, one per block it was not running, burying the line that mattered.
