...
/**
 * The version tag a `removeBuildingBlock` entry must name, refused at both points where a plan can still
 * be corrected: the save path (through [RemoveBuildingBlockMigrationComponentValidator]) and deploy
 * (through [RemoveBuildingBlockMigrationComponentDeployer], which is what a file-deployed plan passes).
 */
