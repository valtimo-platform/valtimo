...
class BuildingBlockMigrationPathResolverTest {
    ...
    fun `should fail when more than one chain of plans leads to the linked version`() {
        // Both a one-plan jump and a two-plan chain reach 1.0.2, so neither is chosen.
    ...
    fun `should fail when two plans declare the same source and target`() {
        // One edge per plan: two plans over the same pair of versions are two chains, not a chain of two steps.
        plan("een", from = block("1.0.0"), to = block("1.0.1"))
    ...
    fun `findPath should still fail when more than one chain of plans leads to the linked version`() {
        // Softening the missing-path refusal says nothing about ambiguity.
        plan("sprong", from = block("1.0.0"), to = block("1.0.2"))
        plan("een", from = block("1.0.0"), to = block("1.0.1"))
        plan("twee", from = block("1.0.1"), to = block("1.0.2"))
