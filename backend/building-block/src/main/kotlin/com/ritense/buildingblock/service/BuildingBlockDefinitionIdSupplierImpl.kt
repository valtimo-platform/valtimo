...
@Component
@SkipComponentScan
class BuildingBlockDefinitionIdSupplierImpl(
    private val buildingBlockDefinitionRepository: BuildingBlockDefinitionRepository
) : BuildingBlockDefinitionIdSupplier {
    override fun getIds(): List<BuildingBlockDefinitionId> {
        return buildingBlockDefinitionRepository.findAll().filter { it.final }.map { it.id }
    }
}
