...
class DocumentObjectenApiSyncExporter(
    ...
    override fun export(request: DocumentDefinitionExportRequest): ExportResult {
        ...
        val syncExport = ExportFile(
            ...
            objectMapper.writer(ExportPrettyPrinter()).writeValueAsBytes(
                DocumentObjectenApiSyncRequest.of(syncConfig)
            )
        )
    ...
    companion object {
        private const val PATH = "config/case/%s/%s/zgw/zaakdetail-sync/%s.zaakdetail-sync.json"
    }
}
