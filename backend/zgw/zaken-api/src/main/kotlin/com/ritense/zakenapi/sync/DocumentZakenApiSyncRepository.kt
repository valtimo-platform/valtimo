...
@Repository
interface DocumentZakenApiSyncRepository : JpaRepository<DocumentZakenApiSync, UUID> {

    @Query(
        "SELECT s FROM DocumentZakenApiSync s " +
    ...
    @Modifying
    @Query(
        "DELETE FROM DocumentZakenApiSync s " +
            "WHERE s.caseDefinitionId.key = :#{#caseDefinitionId.key} " +
            "AND s.caseDefinitionId.versionTag = :#{#caseDefinitionId.versionTag}"
    )
