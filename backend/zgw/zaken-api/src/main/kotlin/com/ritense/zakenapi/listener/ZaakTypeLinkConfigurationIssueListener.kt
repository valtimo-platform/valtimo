...
class ZaakTypeLinkConfigurationIssueListener(
    private val applicationEventPublisher: ApplicationEventPublisher
) {

    @EventListener(ZaakTypeLinkSavedEvent::class)
    fun handleZaakTypeLinkSaved(event: ZaakTypeLinkSavedEvent) {
        applicationEventPublisher.publishEvent(
            CaseConfigurationIssueResolvedEvent(
                event.caseDefinitionId,
                ZaakTypeLinkImporter.ISSUE_TYPE
            )
        )
    }
}
