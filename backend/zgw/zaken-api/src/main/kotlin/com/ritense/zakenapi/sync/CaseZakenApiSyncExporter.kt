...
class CaseZakenApiSyncExporter(
    ...
    override fun export(request: DocumentDefinitionExportRequest): ExportResult {
        ...
        val syncConfig = caseZakenApiSyncRepository.findByCaseDefinitionId(request.caseDefinitionId)
            ?: return ExportResult()
        ...
        val syncExport = ExportFile(
            ...
            objectMapper.writer(ExportPrettyPrinter()).writeValueAsBytes(
                CaseZakenApiSyncRequest.of(syncConfig)
            )
        )

        return ExportResult(syncExport)
    }

    companion object {
        private const val PATH = "config/case/%s/%s/zgw/zaken-api-sync/%s.zaken-api-sync.json"
    }
}
