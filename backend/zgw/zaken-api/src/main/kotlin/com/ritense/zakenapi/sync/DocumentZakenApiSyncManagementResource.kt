...
class DocumentZakenApiSyncManagementResource(
    private val documentZakenApiSyncManagementService: DocumentZakenApiSyncManagementService,
) {
    ...
    ): ResponseEntity<DocumentZakenApiSyncResponse?> {
        val caseDefinitionId = CaseDefinitionId(caseDefinitionKey, caseDefinitionVersionTag)
        val sync = documentZakenApiSyncManagementService.getSyncConfiguration(caseDefinitionId)
            ?: return ResponseEntity.ok(null)
        return ResponseEntity.ok(DocumentZakenApiSyncResponse.of(sync))
    }
    ...
    fun createOrUpdateSyncConfiguration(
        @PathVariable("caseDefinitionKey") caseDefinitionKey: String,
        @PathVariable("caseDefinitionVersionTag") caseDefinitionVersionTag: String,
        @RequestBody syncRequest: DocumentZakenApiSyncRequest,
    ): ResponseEntity<Unit> {
        val caseDefinitionId = CaseDefinitionId(caseDefinitionKey, caseDefinitionVersionTag)
        documentZakenApiSyncManagementService.saveSyncConfiguration(syncRequest.toEntity(caseDefinitionId))
    ...
    fun deleteSyncConfiguration(
        ...
        val caseDefinitionId = CaseDefinitionId(caseDefinitionKey, caseDefinitionVersionTag)
        documentZakenApiSyncManagementService.deleteSyncConfigurationByCaseDefinition(caseDefinitionId)
        return ResponseEntity.ok().build()
    }
}
