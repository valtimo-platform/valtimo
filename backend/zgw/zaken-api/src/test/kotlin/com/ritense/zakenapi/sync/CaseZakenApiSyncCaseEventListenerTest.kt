...
class CaseZakenApiSyncCaseEventListenerTest {

    private val service = mock<CaseZakenApiSyncManagementService>()
    private val listener = CaseZakenApiSyncCaseEventListener(service)
    ...
    fun `duplicates the sync config when a case definition is duplicated`() {
        whenever(service.getSyncConfiguration(eq(sourceId))).thenReturn(
            CaseZakenApiSync(caseDefinitionId = sourceId, assigneeSyncEnabled = true)
        )
