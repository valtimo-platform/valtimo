...
class ZaakDocumentServiceTest {
    ...
    fun `should throw exception when downloading informatieObject that is not related to zaak`() {
        ...
        whenever(zakenApiPlugin.getZaakInformatieObject(zaakUrl, informatieobjectUrl))
            .thenReturn(null)
        ...
        verify(documentenApiService, times(0)).downloadInformatieObject(any<String>(), any<String>())
    }
    ...
    fun `should throw exception when getting informatieObject that is not related to zaak`() {
        ...
        assertEquals("InformatieObject is not related to this Zaak", exception.message)
        verify(documentenApiService, times(0)).getInformatieObject(any<String>(), any<String>())
    }
    ...
    fun `should download informatieObject when it is related to the zaak`() {
        ...
        whenever(documentenApiService.downloadInformatieObject(pluginConfigurationId.toString(), documentId))
        ...
        verify(documentenApiService).downloadInformatieObject(pluginConfigurationId.toString(), documentId)
    }
    ...
    fun `should get informatieObject when it is related to the zaak`() {
        ...
        val informatieObject = createDocumentInformatieObject(informatieobjectUrl)
        whenever(documentenApiService.getInformatieObject(pluginConfigurationId.toString(), documentId))
        ...
        assertEquals(informatieObject, result)
        verify(documentenApiService).getInformatieObject(pluginConfigurationId.toString(), documentId)
    }

    private fun createZaakInformatieObject(zaakUrl: URI, informatieobjectUrl: URI) = ZaakInformatieObject(
        url = createUrl(zaakUrl, "/f5abe5c3-a36c-485b-9935-407e69bae231"),
