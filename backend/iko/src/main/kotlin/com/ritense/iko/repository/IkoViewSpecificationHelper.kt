...
class IkoViewSpecificationHelper {
    ...
    companion object {
        ...
        @JvmStatic
        fun byKey(key: String) = Specification<IkoView> { root, _, cb ->
            cb.equal(root.get<String>(KEY), key)
        }

        @JvmStatic
        fun byIkoRepositoryConfigKey(ikoRepositoryConfigKey: String) = Specification<IkoView> { root, _, cb ->
            cb.equal(root.get<String>(IKO_REPOSITORY_CONFIG).get<String>(KEY), ikoRepositoryConfigKey)
        }

        @JvmStatic
        fun byTitleContains(titlePart: String) = Specification<IkoView> { root, _, cb ->
