...
class IkoViewExporterIntTest @Autowired constructor(
    private val objectMapper: ObjectMapper,
    private val resourceLoader: ResourceLoader,
    private val ikoViewExporter: IkoViewExporter
) : BaseIntegrationTest() {

    @Test
    fun `should export ikoView`(): Unit = runWithoutAuthorization {
        val ikoViewKey = "klant"
        val request = IkoViewExportRequest(ikoViewKey)

        val exportFiles = ikoViewExporter.export(request).exportFiles
        val expectedPath = IkoViewExporter.PATH.format(ikoViewKey, ikoViewKey)
