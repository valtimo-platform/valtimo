...
class ProcessActivityMapperTest {
    ...
    fun setUp() {
        ...
        // Every pair is accepted here, so the assertions are about the alignment and nothing else.
    ...
    fun `should pair every activity with its own rename when a version suffixes all of them`() {
        ...
        // Placed by position alone the three service tasks cross, silently resuming each token in the wrong task.
    ...
    fun `should fall back to the flow position when no name is close enough`() {
        ...
        // Nothing resembles anything, so the positional rule stands — and the activity type still decides.
        assertThat(mapper.suggestActivityMapping(SOURCE, TARGET)).containsExactlyInAnyOrderEntriesOf(
