...
import com.ritense.valtimo.camunda.authorization.CamundaTimerActionProvider
...
class ProcessTimerResourceTest {
    ...
    fun `skip should require COMPLETE permission on the timer`() {
        ...
        verify(authorizationService).requirePermission(
            argThat<EntityAuthorizationRequest<CamundaTimer>> {
                resourceType == CamundaTimer::class.java &&
                    action == CamundaTimerActionProvider.COMPLETE &&
    ...
    fun `getSkippableTimers should only return timer jobs the user may complete`() {
        ...
        whenever(authorizationService.hasPermission(any<EntityAuthorizationRequest<CamundaTimer>>())).thenReturn(true)
    ...
    fun `getSkippableTimers should not return timers the user may not complete`() {
        val processInstanceId = associateInstance()
        stubJobsForInstance(processInstanceId, Triple("job-1", "timer", "timerBoundary"))
        whenever(authorizationService.hasPermission(any<EntityAuthorizationRequest<CamundaTimer>>())).thenReturn(false)
