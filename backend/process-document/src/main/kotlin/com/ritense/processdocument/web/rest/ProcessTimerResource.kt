...
class ProcessTimerResource(
    ...
    private fun hasCompletePermission(timer: CamundaTimer) =
        authorizationService.hasPermission(completePermissionRequest(timer))

    private fun requireCompletePermission(timer: CamundaTimer) =
        authorizationService.requirePermission(completePermissionRequest(timer))

    private fun completePermissionRequest(timer: CamundaTimer) = EntityAuthorizationRequest(
        CamundaTimer::class.java,
        CamundaTimerActionProvider.COMPLETE,
        timer,
    )
    ...
    private fun getTimerJobs(processInstanceId: String): List<TimerJob> = runWithoutAuthorization {
        ...
        rawJobs.map { job ->
            ...
            TimerJob(
                ...
                timer = CamundaTimer.from(job, definition),
            )
