...
class S3Service(
    ...
    fun generatePreSignedPutObjectUrl(fileName: String): URL {
        val putObjectRequest = buildPutObjectRequest(bucketName, getUniqueKey(fileName))
    ...
    private fun getUniqueKey(key: String): String {
        val keyPart1 = key.substringBeforeLast('.', missingDelimiterValue = key)
        val keyPart2 = key.substringAfterLast('.', missingDelimiterValue = "")
        return "$keyPart1-${UUID.randomUUID()}.$keyPart2"
    }
