...
import com.ritense.resource.domain.listener.TemporaryResourceSubmittedToS3EventListener
...
class S3ResourceAutoConfiguration {
    ...
    @Bean
    @ConditionalOnMissingBean(TemporaryResourceSubmittedToS3EventListener::class)
    @ConditionalOnProperty(
        prefix = "valtimo.resource.s3",
        name = ["temp-upload-listener.enabled"],
        havingValue = "true",
        matchIfMissing = false
    )
    fun temporaryResourceSubmittedToS3EventListener(
        temporaryResourceStorageService: TemporaryResourceStorageService,
        s3Service: S3Service,
        documentService: DocumentService
    ): TemporaryResourceSubmittedToS3EventListener {
        return TemporaryResourceSubmittedToS3EventListener(
