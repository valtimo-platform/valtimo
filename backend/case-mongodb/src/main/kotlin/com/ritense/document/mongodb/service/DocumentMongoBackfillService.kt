...
open class DocumentMongoBackfillService(
    private val jpaRepository: JsonSchemaDocumentRepository,
    private val mongoRepository: JsonSchemaDocumentMongoRepository,
    private val objectMapper: ObjectMapper,
) {

    /**
     * Copies all existing [JsonSchemaDocument] rows from the relational database to MongoDB.
    ...
    open fun backfill(pageSize: Int = DEFAULT_PAGE_SIZE): Long {
        ...
        do {
            ...
            val docs = mutableListOf<JsonSchemaDocumentDocument>()
            for (jpaDoc in slice.content) {
                try {
                    val json = objectMapper.writeValueAsString(jpaDoc)
                    val doc = objectMapper.readValue(json, JsonSchemaDocumentDocument::class.java)
                    docs.add(doc.copy(contentText = extractLeafValues(doc.content)))
                } catch (e: Exception) {
                    logger.warn(e) { "Failed to convert document to MongoDB document — skipping" }
                }
            }
        ...
        logger.info { "Backfill complete: $total documents migrated to MongoDB" }
        return total
    }
