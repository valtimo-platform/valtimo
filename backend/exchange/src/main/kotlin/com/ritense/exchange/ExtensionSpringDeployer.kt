...
/**
 * Deploys the *full* backend of a runtime-loaded extension into the host Spring
 ...
 * dropped in the extensions folder.
 *
 ...
 */
class ExtensionSpringDeployer(
    private val extensionManager: ExtensionManager,
) {
    private val applicationContext get() = extensionManager.applicationContext
    ...
    fun deploy(extension: PluginWrapper) {
        // Deploy each extension's backend exactly once, even if both the plugin
        // STARTED event and the context-refreshed hook fire for it.
        if (!deployed.add(extension.pluginId)) return
        val classLoader = extension.pluginClassLoader
        val classes = scanJarForClasses(extension, classLoader)

        runLiquibase(extension, classLoader)
        ...
        // sealed at startup.
        val entityManagerFactory = buildEntityManagerFactory(extension, classLoader, classes)
        registerRepositories(extension, repositories, entityManagerFactory, classLoader)
        registerConfigurationBeans(extension, classes)
        val beanNames = registerComponents(extension, components)
        registerControllerMappings(extension, beanNames, components)
    }
    ...
    private fun buildEntityManagerFactory(
        ...
        return try {
            ...
            logger.info {
                "Built EntityManagerFactory for '${extension.pluginId}'; " +
                    "metamodel managed: ${emf.metamodel.entities.map { it.name }}"
            }
            emf
        } catch (e: Exception) {
            logger.warn { "Could not build EntityManagerFactory for '${extension.pluginId}': ${e.message}" }
            null
        }
    }
     ...
     */
    private fun scanJarForClasses(extension: PluginWrapper, classLoader: ClassLoader): List<Class<*>> {
        val jarPath = extension.pluginPath.toFile()
            ...
            jar.entries().asSequence()
                ...
                .mapNotNull { className ->
                    try {
                        ...
                        logger.debug(t) { "Skipping unloadable class '$className' in '${extension.pluginId}'" }
                        null
                    }
                }
                .toList()
    ...
    private fun runLiquibase(extension: PluginWrapper, classLoader: ClassLoader) {
        val master = findMasterChangelog(extension, classLoader) ?: return
        try {
            val dataSource = applicationContext.getBean(DataSource::class.java)
            ...
            logger.info { "Ran Liquibase changelog '$master' for extension '${extension.pluginId}'" }
        } catch (e: Exception) {
            logger.error(e) { "Failed to run Liquibase for extension '${extension.pluginId}'" }
        }
    }

    /** Convention: a `*-master.xml` under `config/liquibase/` in the plugin jar. */
    private fun findMasterChangelog(extension: PluginWrapper, classLoader: ClassLoader): String? {
        val candidate = JarFile(extension.pluginPath.toFile()).use { jar ->
            jar.entries().asSequence()
                .map { it.name }
    ...
    private fun registerRepositories(
        ...
        if (entityManagerFactory != null) {
            registerSingletonIfAbsent(
                "transactionManager-${extension.pluginId}",
            ...
            try {
                ...
                val emf = when {
                    ...
                    entityManagerFactory != null && isManaged(entityManagerFactory, domainType) -> entityManagerFactory
                        ...
                        logger.warn { "No EntityManagerFactory manages '${domainType.name}'; skipping repository '${repo.name}' of '${extension.pluginId}'" }
                        return@forEach
                    }
                }
                ...
                logger.info { "Registered repository '${repo.name}' for '${extension.pluginId}'" }
            } catch (e: Exception) {
                logger.warn { "Could not register repository '${repo.name}' of '${extension.pluginId}': ${e.message}" }
            }
        }
    }
     ...
     */
    private fun registerConfigurationBeans(extension: PluginWrapper, classes: List<Class<*>>) {
        ...
        configs.forEach { configClass ->
            val instance = try {
                ...
                logger.warn { "Could not instantiate config '${configClass.name}' of '${extension.pluginId}': ${e.message}" }
                return@forEach
            }
            configClass.methods
                ...
                .forEach { method ->
                    ...
                    try {
                        ...
                        logger.info { "Registered @Bean '$name' from '${configClass.simpleName}' for '${extension.pluginId}'" }
                    } catch (e: Exception) {
                        logger.debug { "Skipping @Bean '$name' of '${extension.pluginId}': ${e.message}" }
                    }
                }
        }
    }
     ...
     */
    private fun registerComponents(extension: PluginWrapper, components: List<Class<*>>): List<String> {
        ...
        remaining.forEach { clazz ->
            logger.warn { "Could not create bean '${clazz.name}' of '${extension.pluginId}': ${lastError[beanName(clazz)]}" }
        }
        logger.info { "Registered ${registered.size} bean(s) for '${extension.pluginId}': ${registered.map { it.substringAfterLast('.') }}" }
        return registered
    }
    ...
    private fun registerControllerMappings(
        ...
        logger.info { "Controller candidates for '${extension.pluginId}': ${controllers.map { it.substringAfterLast('.') }}" }
        ...
        controllers.forEach { name ->
            try {
                ...
                logger.info { "Registered controller '$name' request mappings for '${extension.pluginId}'" }
            } catch (e: Exception) {
                logger.warn { "Could not register controller '$name' of '${extension.pluginId}': ${e.message}" }
            }
        }
    }
