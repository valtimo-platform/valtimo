 ...
 *
 * A migration plan (`*.migration.json`) is a package of instructions to migrate the instances of
 * a blueprint (case definition or building block definition) from one version to the next. The
 * plan file itself is imported, exported and auto-deployed by the `case` module, but each
 * component is owned by a different, potentially unlinked module:
 *
 * - `case` module handles the `dataMigration` component
 * - `core` module stores the `processMigration` component
