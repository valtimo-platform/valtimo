...
@Service
@SkipComponentScan
class CaseTagCaseEventListener(
    private val service: CaseTagService
) {

    @RunWithoutAuthorization
    @EventListener(CaseDefinitionCreatedEvent::class)
    fun createCaseTags(event: CaseDefinitionCreatedEvent) {
        if (event.duplicate) {
            service.getCaseTags(event.basedOnCaseDefinitionId!!).forEach { tag ->
                service.create(
                    event.caseDefinitionId, CaseTagCreateRequestDto(
                        key = tag.id.key,
                        title = tag.title,
                        color = tag.color,
                    )
