...
/** Reconstructs a blueprint version's migration plan files from the plan skeleton plus each [MigrationComponentDeployer]'s exported component. */
...
class MigrationPlanExporter(
    ...
    private fun buildPlanJson(migration: CaseDefinitionMigration): ObjectNode {
        ...
        // The target is implied by the folder; the source never is, so it is always written out, key included.
        root.putObject("source")
            .put("key", migration.sourceKey)
            .put("versionTag", migration.sourceVersionTag.toString())
        // A building block plan has neither: it runs when a case migration brings its block onto this version.
