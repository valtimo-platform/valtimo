...
class ImageCaseWidget(
    ...
    val properties: ImageWidgetProperties
) : CaseWidgetTabWidget(
    id, title, icon, color, order, width, highContrast, isCompact, actions, displayConditions
) {
    override fun copy(id: CaseWidgetTabWidgetId) = ImageCaseWidget(
        id = id,
        title = title,
        icon = icon,
        color = color,
    ...
    override fun getUnresolvedValues(): List<String> {
        return (actions.flatMap { it.getUnresolvedValues() } + properties.value).distinct()
    }

    @JsonIgnore
    override fun getExposedValues(resolveValue: (String) -> Any?): Map<String, Any?> {
        return mapOf("value" to resolveValue(properties.value)) +
            actions.flatMap { action -> action.getExposedValues(resolveValue).map { it.key to it.value } }
    }
}
