...
@AllOpen
@Entity
@DiscriminatorValue("person-card")
class PersonCardCaseWidget(
    ...
    override fun copy(id: CaseWidgetTabWidgetId) = PersonCardCaseWidget(
        id = id,
        title = title,
        icon = icon,
        color = color,
    ...
    override fun getUnresolvedValues(): List<String> {
        return (actions.flatMap { it.getUnresolvedValues() } +
            listOfNotNull(
                properties.person.fullName,
                properties.person.birthDate,
                properties.person.bsn,
                properties.person.phone,
                properties.person.email,
                properties.person.city
            )).distinct()
    }
    ...
    override fun getExposedValues(resolveValue: (String) -> Any?): Map<String, Any?> {
        return mapOf(
            "fullName" to resolveValue(properties.person.fullName),
            "birthDate" to properties.person.birthDate?.let { resolveValue(it) },
            "bsn" to properties.person.bsn?.let { resolveValue(it) },
            "phone" to properties.person.phone?.let { resolveValue(it) },
            "email" to properties.person.email?.let { resolveValue(it) },
            "city" to properties.person.city?.let { resolveValue(it) },
        ).filterValues { it != null } + actions
