...
/**
 * The lineage half of the pre-filled migration plan, against a real database: which version a new
 * plan is suggested to migrate *from* when the target records no `basedOnVersionTag` of its own.
 *
 * `MigrationSuggestionServiceTest` covers the choosing; what only a database can show is that the
 * versions are found at all — the deployed set is read with a derived query, and it has to include
 * the inactive `-migrated` definition an upgrade leaves behind, which is exactly the one wanted.
 */
@Transactional
class CaseVersionLineageIntTest @Autowired constructor(
    ...
    fun `a new plan on a version with no recorded predecessor is suggested the version below it`() {
        // The shape a 12-to-13 upgrade leaves behind, and the one the customer configuration has:
        // 1.0.0 arrived by file auto-deploy and 0.1.0-migrated was parked by the upgrade, so neither
        // records a `basedOnVersionTag` and the suggestion used to come back with no source at all —
        // and therefore no title, no key and no components either.
        deploy("0.1.0-migrated", active = false)
        deploy("1.0.0", active = true)
