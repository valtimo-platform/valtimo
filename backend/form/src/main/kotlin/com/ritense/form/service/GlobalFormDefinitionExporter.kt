...
/**
 * Exports a form definition that is not part of a case definition into the `config/global` folder
 * structure. A reduced copy of [FormDefinitionExporter] that writes the form keyed by its name only,
 * without a case definition path segment.
 */
@Transactional(readOnly = true)
class GlobalFormDefinitionExporter(
    private val objectMapper: ObjectMapper,
    private val formDefinitionService: FormDefinitionService
) : Exporter<GlobalFormDefinitionExportRequest> {
    override fun supports() = GlobalFormDefinitionExportRequest::class.java
    ...
    companion object {
        private const val PATH = "config/global/form/%s.form.json"
    }
}
