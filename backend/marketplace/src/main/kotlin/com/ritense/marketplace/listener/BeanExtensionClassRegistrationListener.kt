...
package com.ritense.marketplace.listener

import com.ritense.marketplace.ExtensionClassRegistrationListener
import com.ritense.marketplace.PackageManager
import com.ritense.marketplace.MarketplaceProperties
...
class BeanExtensionClassRegistrationListener(
    ...
    override fun classRegistered(extensionClass: Class<*>) {
        if (isSpringBean(extensionClass)) {
            if (marketplaceProperties.enforceWhitelist) {
    ...
    private fun registerSpringBean(extensionClass: Class<*>) {
        ...
        val extension = packageManager.extensionFactory.create(extensionClass)
    ...
    private fun unregisterSpringBean(extensionClass: Class<*>) {
        ...
        val beanNames = packageManager.applicationContext.getBeansOfType(extensionClass).keys.toMutableSet()
    ...
    private fun checkAllowed(clazz: Class<*>) {
        val classLoader = packageManager.whichPlugin(clazz).pluginClassLoader
        val interfaces = getAllInterfaces(clazz)
        val illegalInterfaces = interfaces
            .map { it.name }
            .filter { !marketplaceProperties.interfaceWhitelist.contains(it) && !classLoader.containsClass(it) }
        ...
        val illegalAnnotations = (interfaces + clazz)
            ...
            .filter { !marketplaceProperties.annotationWhitelist.contains(it) && !classLoader.containsClass(it) }
    ...
    private fun getBeanName(extensionClass: Class<*>): String {
        ...
        val registry = getBeanDefinitionRegistry(packageManager.applicationContext)
