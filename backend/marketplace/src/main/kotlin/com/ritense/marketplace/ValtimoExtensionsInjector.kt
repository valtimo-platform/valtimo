...
class ValtimoExtensionsInjector(
    ...
    packageManager,
    packageManager.applicationContext.autowireCapableBeanFactory as AbstractAutowireCapableBeanFactory
) {

    private val springDeployer = PackageSpringDeployer(packageManager, pluginDeploymentListener)
    private var injected = false

    @PostConstruct
    fun init() {
        packageManager.addPluginStateListener(this)
    }

    /**
     * Deploy started packages only once the host context is fully refreshed, so
    ...
    override fun injectExtensions() {
        // Register each started package independently and in isolation: a
        // single package that fails to wire (e.g. a plugin whose internal
        ...
        springPluginManager.startedPlugins.toList().forEach { extension ->
            try {
                registerPackage(extension)
            } catch (e: Exception) {
                logger.error(e) { "Failed to register package '${extension.pluginId}'; marking it FAILED" }
                packageManager.fail(extension, e)
            }
        }
    }
    ...
    override fun pluginStateChanged(event: PluginStateEvent) {
        try {
            when (event.pluginState) {
                STARTED -> registerPackage(event.plugin)
                STOPPED -> unregisterExtension(event.plugin)
                FAILED -> try {
                    unregisterExtension(event.plugin)
                } catch (t: Throwable) {
                    logger.debug(t) { "Error while unregistering package ${event.plugin.pluginId}" }
                }

                else -> {}
            }
        } catch (e: Exception) {
            packageManager.fail(event.plugin, e)
        }
    }

    fun registerPackage(pkg: PluginWrapper) {
        // Deploy the package's whole backend (services, repositories, REST
    ...
    fun unregisterExtension(extension: PluginWrapper) {
        packageManager.getExtensionClassNames(extension.pluginId).forEach { extensionClassName ->
