...
class WhitelistSpringExtensionFactory(
...
) : SpringExtensionFactory(packageManager, true) {
    ...
    override fun <T : Any?> createWithSpring(extensionClass: Class<T>, applicationContext: ApplicationContext): T {
        val extension = packageManager.whichPlugin(extensionClass)
        val extensionClassNames = packageManager.getExtensionClassNames(extension.pluginId)

        if (marketplaceProperties.enforceWhitelist) {
            val illegalConstructorParameters = extensionClass.constructors
                .flatMap { it.parameterTypes.toList() }
                .filter { !marketplaceProperties.autowireWhitelist.contains(it.name) && !extensionClassNames.contains(it.name) }
