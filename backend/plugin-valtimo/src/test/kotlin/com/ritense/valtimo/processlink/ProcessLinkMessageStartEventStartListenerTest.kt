...
class ProcessLinkMessageStartEventStartListenerTest {

    lateinit var pluginProcessLinkRepository: PluginProcessLinkRepository
    lateinit var pluginService: PluginService
    lateinit var listener: ProcessLinkMessageStartEventStartListener

    @BeforeEach
    fun setup() {
        pluginProcessLinkRepository = mock()
        pluginService = mock()
        listener = ProcessLinkMessageStartEventStartListener(pluginProcessLinkRepository, pluginService)
    }
    ...
    fun `should invoke plugin service for each matching process link`() {
        ...
        whenever(execution.currentActivityId).thenReturn("messageStart1")
        ...
        whenever(
            pluginProcessLinkRepository.findByProcessDefinitionIdAndActivityIdAndActivityType(
                "proc-def-1", "messageStart1", ActivityTypeWithEventName.MESSAGE_START_EVENT_START
            )
