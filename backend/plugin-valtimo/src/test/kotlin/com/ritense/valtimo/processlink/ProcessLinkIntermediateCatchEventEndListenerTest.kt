...
import org.operaton.bpm.engine.ActivityTypes.INTERMEDIATE_EVENT_CATCH
import org.operaton.bpm.engine.delegate.DelegateExecution

class ProcessLinkIntermediateCatchEventEndListenerTest {

    lateinit var pluginProcessLinkRepository: PluginProcessLinkRepository
    lateinit var pluginService: PluginService
    lateinit var listener: ProcessLinkIntermediateCatchEventEndListener

    @BeforeEach
    fun setup() {
        pluginProcessLinkRepository = mock()
        pluginService = mock()
        listener = ProcessLinkIntermediateCatchEventEndListener(pluginProcessLinkRepository, pluginService)
    }
    ...
    fun `should invoke plugin service for each matching process link`() {
        ...
        whenever(execution.currentActivityId).thenReturn("messageCatch1")
        ...
        whenever(
            pluginProcessLinkRepository.findByProcessDefinitionIdAndActivityIdAndActivityType(
                "proc-def-1", "messageCatch1", ActivityTypeWithEventName.INTERMEDIATE_CATCH_EVENT_END
            )
